...
import { MovieCountByEachRate } from '../../pages/Dashboard';
import { IoIosArrowBack, IoIosArrowForward } from 'react-icons/io';

interface MovieTableAmoutEachRateProps {
    movieData: MovieCountByEachRate[];
}

const MovieTableAmoutEachRate: React.FC<MovieTableAmoutEachRateProps> = ({ movieData }) => {
            ...
            <table className="table-auto min-w-full divide-y divide-gray-200 drop-shadow-md">
                <thead>
                    <tr className="bg-white">
                        <th className="px-6 py-3 text-left text-lg font-medium text-gray-500">Rating</th>
                        <th className="px-6 py-3 text-left text-lg font-medium text-gray-500">Movie Count</th>
                    </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                    {currentItems.map((item) => (
                        <tr key={item.rating} className='hover:bg-blue-50'>
                            <td className="px-6 py-4 whitespace-nowrap text-md font-medium text-gray-900">{item.rating}</td>
...
export default MovieTableAmoutEachRate